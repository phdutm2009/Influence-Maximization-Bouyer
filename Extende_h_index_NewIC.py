...
def extend_h_index(G,n):
    ehi = h_index(G,n)
    for neighbour in G[n]:
        ehi += h_index(G,neighbour)
    return ehi
G_extend_h_index = {}
for node in G.nodes():
    G_extend_h_index[node]=extend_h_index(G,node)
...
#Sort nodes
sorted_G_extend_h_index = {k: v for k, v in sorted(G_extend_h_index.items(), key=lambda item: item[1],reverse=True)}
...
top_50 = list(sorted_G_extend_h_index)[:50]
top_50
